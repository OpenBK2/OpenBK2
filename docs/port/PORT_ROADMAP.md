...
| [ ] | **This is where the Linux build stops as of 2026-08-27.** A resource that fails to load crashes rather than degrading: `CGrannyMeshLoader::Recalc` guards the loader but not the file inside it, so a missing, truncated or corrupt `.gr2` reaches Granny as a null pointer | [A failed resource load is a crash](#a-failed-resource-load-is-a-crash) |
...
**Reached on Linux 2026-08-27, and it is now the thing that stops the game.**
Once the main menu teardown crash was fixed in `c0b327375`, the run gets into a
mission, loads a 2474 object segment, and dies here on the first model. The
route in is different from the Windows one and the fault is identical: the stub
returns null from `GrannyReadEntireFileFromMemory` rather than a missing file
doing it, and nothing between there and `pData->Models[0]` looks.

That makes this item worth more than its tier suggests. Fixing the guard does
not get Granny working, but it turns a crash into a model that fails to draw,
which is the difference between a build that cannot be explored and one that
can. Everything past the first `.gr2` is currently unreachable and untested for
that reason alone.
...
Read 2026-08-27, from the first run that got far enough to produce one. Before
`c0b327375` the game never reached a model at all.

**The whole log is three lines.**
...
`fopen` is `"w"`, so this is one run and not an accumulation. Two distinct entry
points out of 54, and the third call is already the failure: the stub returned
...
because everything past the first `.gr2` is unreachable while a failed load is
fatal. The order to work in is therefore:

1. `GrannyReadEntireFileFromMemory`, which is what a `.gr2` parser has to
   produce in the first place.
2. `GrannyGetFileInfo`, plus the `granny_file_info` and `granny_model` layout
   the caller walks by hand.
3. Guard the load path so a null degrades instead of crashing, then run again
   and read a longer log.

Until step 3 is done, every new Granny function costs a full run to discover.

**What the tree references, for contrast.** A `grep` for `Granny*(` across the
whole of `Sources` finds 66 distinct names at 189 call sites, which does not
match the 54 the stub defines. The twelve are all explained, and none of them is
a gap:

- **Nine are editor only**, in `ED_B2_M1` and `ED_Common`:
  `GrannyConvertSingleObject`, `GrannyMemberHasPointers`, `GrannyGetVersionString`,
  `GrannyGetMeshVertices`, `GrannyGetMeshVertexType`, `GrannyGetMeshVertexCount`,
  `GrannyGetMeshIndices`, `GrannyGetMeshIndexCount`, `GrannyGetMeshBytesPerIndex`.
  Those modules are not built, which is why the linker never asked for them.
- **`GrannyPlayControlledAnimation` is commented out**, at `3Dmotor/GAnimation.cpp:450`.
- **`GrannyReplacementAlloc` and `GrannyReplacementDealloc` are not Granny's**.
  They are the engine's own callbacks, defined in `3Dmotor/GrannyMemoryMap.cpp`
  and handed to `GrannySetAllocator`. They match the `Granny` prefix and nothing
  else.

`nm -D --undefined-only` over every installed `.so` and `Game` finds **zero**
undefined `Granny` symbols, which is the check that settles it: the stub's 54
are exactly what the built engine links against.

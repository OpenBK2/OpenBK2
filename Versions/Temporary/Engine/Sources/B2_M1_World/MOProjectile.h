...
class CMOProjectile : public CMapObj
{
	...
	~CMOProjectile();
public:
	CMOProjectile() : fDamage( -1 ), bModelExists( false ), bHitTarget( false ), nShell( 0 ), bTraceTargetIntersection( false ), vRotationSpeedRad( VNULL3 ) { }

	bool Create( const struct SAINewProjectileUpdate *pUpdate, const NDb::SProjectile *pProjectile, const CVec3 &vVisPos, const CQuat &qRot, const NDb::SComplexEffect *_pTrajectoryEffect, const CVec3 &vRotationSpeedRad );

...
// The text editors as they have always been -- CScriptEditor and
// CTextEditorDialog -- behind the boundary, the dispatcher in front of both
// implementations, and the part of the script editor that is not drawing, which
// the wx one uses as well.
...
namespace NTextEditor
{
	...
	{
		CScriptEditor scriptEditor( 0, ToCWnd( pParent ) );
		if ( !rszTitle.empty() )
		{
			scriptEditor.SetTitle( rszTitle );
		}
		scriptEditor.SetText( rszText );
		scriptEditor.EnableEdit( bEnableEdit );
		if ( ( scriptEditor.DoModal() == IDOK ) && bEnableEdit )
		{
			( *pszNewText ) = scriptEditor.GetText();
			return true;
		}
		return false;
	}
	...
	{
		CTextEditorDialog textEditorDialog( ToCWnd( pParent ) );
		if ( !rszTitle.empty() )
		{
			textEditorDialog.SetTitle( rszTitle );
		}
		textEditorDialog.SetType( typeTEMnemonics.Get( rszEditor ) );
		textEditorDialog.SetText( rszText );
		textEditorDialog.EnableEdit( bEnableEdit );
		if ( ( textEditorDialog.DoModal() == IDOK ) && bEnableEdit )
		{
			textEditorDialog.GetText( pszNewText );
			return true;
		}
		return false;
	}

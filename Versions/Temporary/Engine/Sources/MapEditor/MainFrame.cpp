...
CMainFrame::CMainFrame() : nFreeToolbarID( AFX_IDW_TOOLBAR + 9 )
{
...
{
	NMainFrameShared::OpenResource( rszResourceName );
}
...
void CMainFrame::OnUpdateUserCommand( CCmdUI *pCmdUI )
{
	bool bEnable = false;
	bool bChecked = false;
	NMainFrameShared::UpdateUserCommand( pCmdUI->m_nID, &bEnable, &bChecked );
	...
	unsigned nMenuID = 0;
	if ( const SUserData::CRecentList *pRecentList = NMainFrameShared::GetRecentList( pCmdUI->m_nID, &nMenuID ) )
	{
			...
			for ( int nMenuItemIndex = 0; nMenuItemIndex < nMenuItemCount; ++nMenuItemIndex )
			{
				if ( CMenu *pSubMenu = pMenu->GetSubMenu( nMenuItemIndex ) )
				{
					if ( ( pSubMenu->GetMenuItemCount() > 0 ) && ( pSubMenu->GetMenuItemID( 0 ) == nMenuID ) )
					{
						...
						{
							pSubMenu->InsertMenu( -1, MF_BYPOSITION, nMenuID, NMainFrameShared::GetRecentEmptyLabel().c_str() );
						}
						break;
					}
				}
...
{
	pCmdUI->Enable( NMainFrameShared::HasHelpFile( szHelpFilePath ) );
}
...
{
	return NMainFrameShared::SaveChanges( bShowConfirmDialog, [this]() { ReloadData(); } );
}
...
void CMainFrame::PostCommand( unsigned nCommandID )
{
	PostMessage( WM_COMMAND, nCommandID, 0 );
}

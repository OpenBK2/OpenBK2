...
-- libgranny2_x64.so: all 63 expected exports present, 63 in total
...
library's export table and fails unless it holds exactly the 63 names in
...
**60 implemented, 3 stubs, 0 reached-but-missing.** A run of the game logs no
...
### Implemented but not reached in a normal run (25)
...
| `GrannyGetMeshVertexCount`, `GrannyGetMeshVertices`, `GrannyGetMeshVertexType`, `GrannyGetMeshIndexCount`, `GrannyGetMeshIndices`, `GrannyGetMeshBytesPerIndex` | **editor only** | `TraceModel` in `ED_B2_M1`, which ray-casts against model geometry for picking. Implemented, and compared against the DLL over the whole corpus |
| `GrannyGetVersionString`, `GrannyVersionsMatch_` | **editor only** | `PrintGrannyVersions` in `ED_B2_M1Dll.cpp`, logging the DLL version at editor startup and checking it against the header's |
...
internal and none of them is among the 63 exports, and a static link needs no DLL
...
library says which of the 63 are actually reached, in what order, and with what
arguments. The engine references 54 of them and the map editor the other nine,
but a reference is not a call: some sit
...
`include/gr2/granny.h`, so the 63 signatures cannot drift apart, and it is

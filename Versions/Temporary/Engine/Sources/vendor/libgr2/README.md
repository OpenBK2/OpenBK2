...
-- libgranny2_x64.so: all 61 expected exports present, 61 in total
...
library's export table and fails unless it holds exactly the 61 names in
...
**58 implemented, 3 stubs, 0 reached-but-missing.** A run of the game logs no
...
### Implemented but not reached in a normal run (23)
...
internal and none of them is among the 61 exports, and a static link needs no DLL
...
library says which of the 61 are actually reached, in what order, and with what
arguments. The engine references 54 of them and the map editor the other seven,
...
`include/gr2/granny.h`, so the 61 signatures cannot drift apart, and it is

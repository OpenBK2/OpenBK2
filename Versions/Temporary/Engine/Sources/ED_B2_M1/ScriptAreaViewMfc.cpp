...
// The script area palette as it has always been: CScriptAreaWindow, a
// CResizeDialog over IDD_TAB_MI_SCRIPT_AREA with two radio buttons, two push
// buttons and a two column report list.
//
// The window class is untouched apart from taking its command dispatch from
// CScriptAreaCommands -- the whole of it, in this one case. This is its
// creation site, moved out of MapInfoEditor.

namespace NScriptAreaView
{
	CWnd* CreateMfc( CDefault3DTabWindow *pTabWindow )
	{
		CScriptAreaWindow *pDialog = pTabWindow->AddNewTab( static_cast<CScriptAreaWindow*>( 0 ) );
		if ( pDialog == 0 )
		{
			return 0;
		}
		...
		pDialog->Create( CScriptAreaWindow::IDD, pTabWindow );

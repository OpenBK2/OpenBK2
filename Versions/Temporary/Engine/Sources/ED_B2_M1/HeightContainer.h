...
class CHeightContainer
{
	...
	std::vector<CArray2D<uint32_t> > blackPlaneStackList;			// стеки однобитных плоскостей ( по sizeof(uint32_t) на стек ) (черные)
	std::vector<CArray2D<uint32_t> > redPlaneStackList;				// стеки однобитных плоскостей ( по sizeof(uint32_t) на стек ) (красные)
	std::vector<uint32_t> markedBitslList;										// набор битов помарканной точки (берется из черной плоскости)
	std::vector<uint32_t> filledBitsList;											// набор битов указывающий заполненность ( если последнее число == 0, то можно сократить количество плоскостей ) (общее для обоих типов плоскостей)
	...
	void FillPlane( int nPlaneIndex,
									const std::vector<CVec2> &rBlackPolygon,
									const std::vector<CVec2> &rRedPolygon );										// закрасить плоскость
	//
	void GetBits( std::vector<uint32_t> *pBitsList, const int x, const int y );
	void AddBitsToString( std::string *pszMessage, const uint32_t dwBits ) const;
	//
...
public:
	...
	//
	void InsertPolygon( const std::vector<CVec2> &rBlackPolygon,
											const std::vector<CVec2> &rRedPolygon,

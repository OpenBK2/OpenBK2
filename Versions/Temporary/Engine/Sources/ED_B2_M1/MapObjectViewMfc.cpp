#include "stdafx.h"

#include "MapObjectView.h"
#include "MapObjectWindow.h"
#include "ED_B2_M1Dll.h"

#include <cstdlib>

// The map object palette as it has always been: CMapObjectWindow, a
// CResizeDialog over IDD_TAB_MI_MAPOBJECT_NO_BUTTONS with a player combo, a
// direction group, a filter combo and an object list.
//
// The window class is untouched apart from taking the edit-parameter half of
// its command dispatch from CMapObjectCommands. This is its creation site,
// moved out of MapInfoEditor.

namespace NMapObjectView
{
	CWnd* CreateMfc( CDefault3DTabWindow *pTabWindow )
	{
		// Constructed before AddNewTab rather than by it, because the
		// constructor takes an argument. The tab list owns it from there, which
		// is what AddNewTab is being told here.
		CMapObjectWindow *pDialog = pTabWindow->AddNewTab( new CMapObjectWindow( false ) );
		if ( pDialog == 0 )
		{
			return 0;
		}
		// The dialog template lives in this module's resources, not the
		// executable's, so the resource handle is swapped for the call and put
		// back. Exactly what the creation site in MapInfoEditor did.
		AfxSetResourceHandle( theEDB2M1Instance );
		pDialog->Create( CMapObjectWindow::IDD_NO_BUTTONS, pTabWindow );
		AfxSetResourceHandle( AfxGetInstanceHandle() );
		return pDialog;
	}


	CWnd* Create( CDefault3DTabWindow *pTabWindow )
	{
#ifdef OBK2_WITH_WX
		// The same flag every migrated piece follows, so a session runs either
		// the MFC set or the wx set.
		const char *pszUseWx = std::getenv( "OBK2_WX_DIALOGS" );
		if ( pszUseWx != 0 && pszUseWx[0] != '0' && pszUseWx[0] != '\0' )
		{
			return CreateWx( pTabWindow );
		}
#endif
		return CreateMfc( pTabWindow );
	}
}

...
//
// A partial dispatch, like the height palette's: this one answers five commands
// of its own -- the object storage query, the clear-selection command, and the
// object list's three context menu items -- so it overrides HandleCommand and
// UpdateCommand and falls through to here for the edit-parameter pair.
typedef CEditParameterCommands<CMapObjectMultiState::SEditParameters> CMapObjectCommands;
